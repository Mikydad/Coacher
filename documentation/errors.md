...
## 25) Crashlytics shows "3 unprocessed crashes" / empty Issues list — dSYMs never uploaded
- **Where**: Firebase console → Crashlytics → `sidepal` iOS app. Found 2026-09-12 while checking whether Crashlytics works at all.
- **Error / symptom**: Banner "This app has 3 unprocessed crashes. Upload 2 dSYM files to process them." Issues list empty, crash-free users 100% — i.e. three real production crashes on build 1.0.1 (2) that nobody could see. Missing UUIDs: `0029D77A-8E97-3384-865D-56C2B57EA879` (required, 2 events), `0626C576-D57A-3723-8AC5-BF1BE0BECCC7` (required, 1), `4C4C44FB-5555-3144-A127-83AEB281A990` (optional — Flutter engine).
- **Root cause**: `ios/Runner.xcodeproj` had no Crashlytics run-script build phase, so no archive ever uploaded its symbols. Dart-level exceptions symbolicate client-side, but the two documented production crashes (#19, #20) were both *native* boot crashes — exactly the kind that needs dSYMs. Also: no way to fire a test event, so the pipeline had never been verified end to end.
- **Fix applied**: (a) New LAST build phase "Upload dSYMs to Crashlytics" in `project.pbxproj`: runs `${PODS_ROOT}/FirebaseCrashlytics/run -gsp Runner/GoogleService-Info.plist` (Runner.app.dSYM + build event, validates synchronously) then sweeps `${DWARF_DSYM_FOLDER_PATH}` with `upload-symbols` in the background so Flutter's `App.framework.dSYM` (Dart AOT) and `Flutter.framework.dSYM` go too. Skipped for Debug + simulator (collection is off there anyway). `ENABLE_USER_SCRIPT_SANDBOXING` is already `NO`, which the upload needs. (b) Hidden smoke test: tester mode ON → long-press the version footer on About & Support → "Send non-fatal" (event arrives in minutes, app keeps running) or "Crash app" (report uploads on next launch). `CrashlyticsTestSink` in `lib/features/settings/application/`. The dialog says outright when collection is off (debug builds).
- **Recovering the 3 stuck crashes**: their dSYMs are not on the dev Mac (no Xcode archive; `build/ios/Release-iphoneos` has been rebuilt since). App Store Connect keeps them: TestFlight → build 1.0.1 (2) → *Download dSYM*. **Mind the app split**: the same-day Apple-team migration (`406d4dd`) registered a NEW Firebase iOS app "SidePal iOS" (`1:8992228827:ios:a46882654a19b65a9553fe`, bundle `com.elaristechnologies.sidepal`) — the current plist points there and so does the build phase, which is right for future builds. The three old crashes sit under the OLD app `sidepal` (`1:8992228827:ios:a52900e663250d169553fe`, bundle `io.sidepal.app`), and build 1.0.1 (2) was shipped from the OLD Apple team, so its dSYM download lives under that team in ASC. Upload with the app id, not the plist: `ios/Pods/FirebaseCrashlytics/upload-symbols -ai 1:8992228827:ios:a52900e663250d169553fe -p ios <downloaded.zip>`. Verify on the old app's dSYMs tab (`…/crashlytics/app/ios:io.sidepal.app/dsyms`).
- **Status**: Fixed for future builds (unverified until the next archive — check the dSYMs tab shows no "Missing (required)" for that version). Old crashes: pending the ASC download. **2026-09-24 update:** the NEW app showed "3 unprocessed crashes, upload 1 dSYM" on 1.0.2 (3)/(4) even with the build phase in place — so the background `upload-symbols` sweep is not reliably finishing (likely killed when the phase exits). Uploaded by hand from the local archives (`~/Library/Developer/Xcode/Archives/2026-09-20/`): build 4 `App.framework.dSYM` (UUID 0C7143A3, shared with build 3) + `Runner.app.dSYM` (E0FCC096), build 3 `Runner.app.dSYM` (2211B603), all accepted. Command that works: `ios/Pods/FirebaseCrashlytics/upload-symbols -gsp ios/Runner/GoogleService-Info.plist -p ios <dSYM>`. Consider making the sweep synchronous (drop the trailing `&`) — it adds ~30 s to an archive.
