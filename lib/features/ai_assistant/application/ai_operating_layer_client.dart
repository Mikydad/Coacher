...
// ─── System prompt ────────────────────────────────────────────────────────────
//
// The Coach system prompt is SERVER-OWNED (fix-wave Phase 5, §8 S1;
// functions/src/coach_prompts.ts): aiChat and aiChatStream DROP any client
// `system` message for chat-class purposes and prepend their own, chosen by
// purpose (coach_agent / coach_agent_voice / coach_agent_stream / …). The
// client used to ship a full copy that the server ignored; it drifted from
// the server text and one test pinned the dead copy (AI chat fix plan
// Phase 7.1). Now the client sends a one-line placeholder — enough to keep
// the message shape valid and honest if a purpose is ever not server-owned.
const String kServerOwnedPromptPlaceholder =
    'You are Coach, the in-app coach of SidePal. Your full instructions for '
    'this purpose are supplied by the server.';

/// Streamed (answer-only) turns: no tools exist on that endpoint.
const String _kStreamPlaceholder =
    '$kServerOwnedPromptPlaceholder This is an answer-only turn: you cannot '
    'look up other days or change anything.';

/// Messages for the aiChatStream endpoint (voice Level 2 + typed queries):
/// placeholder system line, prior turns, then the context-grounded user
/// prompt. tool_calls entries (assistant maps without a text content) are
/// skipped — the streaming endpoint speaks plain role/content only.
List<Map<String, String>> buildConversationalStreamMessages(
  ...
  return [
    {'role': 'system', 'content': _kStreamPlaceholder},
...
class ProxyAiOperatingLayerClient implements AiOperatingLayerClient {
  ...
  Future<AiPlannedChanges> parseIntent(AiOperatingLayerPayload payload) async {
    ...
    final messages = <Map<String, dynamic>>[
      // Server-owned prompt: the purpose (coach_agent / coach_agent_voice)
      // selects it, including the spoken-turn addendum.
      {'role': 'system', 'content': kServerOwnedPromptPlaceholder},
