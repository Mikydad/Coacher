...
class IsarAnalyticsRepository implements AnalyticsRepository {
  ...
  Future<void> logEvent(AnalyticsEvent event) async {
    ...
    if (existingByKey != null &&
        existingByKey.updatedAtMs >= event.updatedAtMs) {
      return;
    }
    await _isar.writeTxn(() async {
      ...
      await _isar.isarAnalyticsEvents.putByEventId(
        IsarAnalyticsEvent.fromDomain(event),
      );
    });
  ...
  Future<List<AnalyticsEvent>> listEvents({
    ...
    final rows = await _isar.isarAnalyticsEvents
        .where()
  ...
  Future<void> hydrateRemoteEvents({List<String>? entityIds}) async {
    ...
    for (final incoming in remote) {
      if (entityIds != null &&
          ...
          !entityIds.contains(incoming.entityId)) {
        continue;
      }
      final existingById = await _isar.isarAnalyticsEvents
          .filter()
          .eventIdEqualTo(incoming.id)
          .findFirst();
      if (existingById != null &&
          existingById.updatedAtMs >= incoming.updatedAtMs) {
        continue;
      }
      final existingByKey = await _isar.isarAnalyticsEvents
          .filter()
          ...
          .findFirst();
      if (existingByKey != null &&
          existingByKey.updatedAtMs >= incoming.updatedAtMs) {
        continue;
      }
      await _isar.writeTxn(() async {
        if (existingByKey != null) {
  ...
  Future<void> upsertStatsCache(AnalyticsStatsCache stats) async {
    ...
    await _isar.writeTxn(() async {
      await _isar.isarAnalyticsStats.putByStatsId(
        IsarAnalyticsStats.fromDomain(stats),
      );
    });
  ...
  Future<void> hydrateRemoteStatsCache({List<String>? scopeIds}) async {
    ...
    for (final incoming in remote) {
      if (scopeIds != null &&
          scopeIds.isNotEmpty &&
          !scopeIds.contains(incoming.scopeId)) {
        continue;
      }
