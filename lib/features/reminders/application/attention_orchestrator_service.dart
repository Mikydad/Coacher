...
class AttentionOrchestratorService {
  ...
  /// OS notification ID currently active per entity (entityId → notif id).
  ...
  /// Timestamps of snoozes per entity for repeatedSnoozePattern detection.
  final Map<String, List<int>> _snoozeTimestampsMs = {};

  /// Consecutive ignored count per entity — incremented on each ignored
  /// follow-up, reset to 0 on any positive interaction (open/snooze/dismiss).
  ...
  Future<AttentionDecision> evaluate(ReminderIntent intent) async {
    ...
    final decision = AttentionOrchestrator.evaluate(
      ...
      consecutiveIgnoredCount:
          _ignoredCountByEntity[intent.entityId] ?? 0,
    );
  ...
  Future<void> onInteractionReceived(
    ...
    switch (type) {
      case NotificationInteractionType.opened:
        _ignoredCountByEntity.remove(entityId); // reset on positive interaction
      ...
      case NotificationInteractionType.snoozed:
        _ignoredCountByEntity.remove(entityId); // snooze counts as engagement
        await _recordSnooze(entityId);

      case NotificationInteractionType.dismissed:
        _ignoredCountByEntity.remove(entityId); // explicit dismiss = engagement
        await _logFatigueEvent(
      ...
      case NotificationInteractionType.ignored:
        // Increment consecutive ignored count for back-off evaluation.
        _ignoredCountByEntity[entityId] =
            (_ignoredCountByEntity[entityId] ?? 0) + 1;
        await _logFatigueEvent(
    ...
    switch (decision.outcome) {
      ...
      case AttentionOutcome.delayed:
        ...
        try {
          ...
          _activeNotificationIds[intent.entityId] = notifId;
  ...
  Future<void> _cancelActiveNotification(String entityId) async {
    final id = _activeNotificationIds.remove(entityId);
    if (id != null) {
      try {
  ...
  Future<void> _recordSnooze(String entityId) async {
    ...
    final timestamps = _snoozeTimestampsMs.putIfAbsent(entityId, () => []);
    timestamps.add(now);
    ...
    if (timestamps.length >= kSnoozePatternThreshold) {
      await _logFatigueEvent(
        type: AnalyticsEventType.repeatedSnoozePattern,
        entityId: entityId,
        reason: '${timestamps.length} snoozes in 24h',
      );
    }
  }
